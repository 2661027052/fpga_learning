...
每个 `key_debounce` 实例将异步按键输入同步化、消抖并产生单周期脉冲，脉冲触发对应 LED 寄存器的翻转。

Each `key_debounce` instance synchronizes, debounces the async key input, and generates a single-cycle pulse. The pulse toggles the corresponding LED register.

## 仿真说明 | Simulation

```bash
# ModelSim (需同时编译 key_debounce 源码 / compile key_debounce source as well)
cd sim
...
1. **单个按键按下/释放** — 验证基本功能
   **Single key press/release** — basic functionality verified
2. **同一按键再次按下** — 验证 LED 翻转行为
   **Same key pressed again** — LED toggle behavior verified
3. **多按键同时按下** — 验证独立控制
   **Multiple keys pressed simultaneously** — independent control verified
4. **所有按键逐个测试** — 验证通道完整性
   **All keys tested one by one** — channel integrity verified

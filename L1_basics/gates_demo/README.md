...
| a         | I        | 1          | 输入 A / Input A |
| b         | I        | 1          | 输入 B / Input B |
| en        | I        | 1          | 三态门使能（1=导通，0=高阻）/ Tri-state enable (1=on, 0=high-Z) |
| s         | I        | 1          | MUX 选择（0=选A，1=选B）/ MUX select (0=A, 1=B) |
...
Testbench 自动遍历 4 种 A/B 输入组合，每种组合自检所有门电路输出，验证真值表正确性。

The testbench exhausts all 4 A/B input combinations and self-checks all gate outputs for each combination, verifying truth table correctness.
